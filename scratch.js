        ...
        const camera = new THREE.OrthographicCamera(-trackViewSize * aspect, trackViewSize * aspect, trackViewSize, -trackViewSize, 1, 2000);
        ...
        function calculatePitStrategy(data) {
            ...
            if (pitStatus === "PITTING" || pitStatus === "IN PIT LANE") return { text: "IN PITS", color: "#000", bgColor: "var(--fia-yellow)", border: "none" };
            ...
            if (isRaining && onDryTyres) return { text: "BOX FOR WETS", color: "#FFF", bgColor: "var(--f1-red)", border: "none" };
            if (!isRaining && (currentCompound.includes("INTER") || currentCompound.includes("WET"))) return { text: "BOX FOR SLICKS", color: "#FFF", bgColor: "var(--f1-red)", border: "none" };
            if (maxWear > 70) return { text: "PUNCTURE RISK: BOX", color: "#FFF", bgColor: "var(--f1-red)", border: "none" };
            else if (maxWear > 55) return { text: "PIT WINDOW OPEN", color: "#000", bgColor: "var(--fia-yellow)", border: "none" };
            else if (maxWear > 45) return { text: "PREPARE TO BOX", color: "var(--fia-yellow)", bgColor: "var(--f1-dark)", border: "1px solid var(--fia-yellow)" };
            if (lapsRemaining <= 2 && maxWear < 70 && lapsRemaining > 0) return { text: "PUSH TO END", color: "#000", bgColor: "var(--fia-purple)", border: "none" };
            return { text: "STAY OUT", color: "var(--fia-green)", bgColor: "var(--f1-dark)", border: "1px solid var(--fia-green)" };
        }
        ...
        function promptAddSector() {
            ...
            document.getElementById('sec2-input').value = Number(currentCustomSectors[1]) - Number(currentCustomSectors[0]);
        ...
        function getLeaderboardDriver(data, carIndex) {
            return Array.isArray(data?.leaderboard) ? data.leaderboard.find(driver => driver.carIndex === carIndex) : null;
        }
        ...
        function updateMapOverlays() {
            ...
            carMeshes.forEach(mesh => {
                ...
                if (showPitCallouts || !inPit) {
                    labelsHtml += `<div class="driver-callout" style="left:${point.x}px; top:${point.y}px; color:${teamColor};">${escapeHtml(abbr)}</div>`;
                }
            });
            ...
            if (selectedCarIndex !== null) {
                ...
                if (selected) {
                    ...
                    const team = driver.teamName || latestMapData?.allCars?.[selectedCarIndex]?.teamName || 'Unknown';
                    const teamColor = getTeamColorCss(driver, latestMapData);
                    labelsHtml += `<div class="track-popup" style="left:${selected.x}px; top:${selected.y}px; color:${teamColor};">
                        <div class="name">${escapeHtml(name.toUpperCase())}</div>
                        <div class="meta"><span>Team</span><span>${escapeHtml(team)}</span></div>
                        <div class="meta"><span>Gap Front</span><span>${escapeHtml(getPopupGapText(latestMapData, driver))}</span></div>
                    </div>`;
                } else {
            ...
            if (typeof customSectorMeshes !== 'undefined') {
                customSectorMeshes.forEach((mesh, idx) => {
                    if (mesh.visible) {
                        ...
                        labelsHtml += `<div style="position: absolute; left:${point.x}px; top:${point.y}px; transform: translate(-50%, -150%); color:${color}; font-size: 0.7rem; font-family: 'Roboto Mono', monospace; font-weight: bold; pointer-events: none; text-shadow: 0 0 4px #000;">${labelText}</div>`;
                    }
                });
            }
        ...
        function getKinematicG(speedKmh, currentMs) {
            ...
            if (!Number.isFinite(speedMs) || lastSpeedMs === null || lastGSampleTime === 0 || currentMs < lastGSampleTime) {
                lastSpeedMs = speedMs;
        ...
        function connectWebSocket(urlIndex = 0) {
            ...
            ws.onopen = () => { setText('ws-status', 'CONNECTED'); setStyleById('ws-status', 'color', 'var(--fia-green)'); };
            ws.onerror = () => {
        ...
        function handleTelemetryMessage(event) {
            ...
            const playerLbData = data.leaderboard ? data.leaderboard.find(d => d.carIndex === data.playerIndex) : null;
            ...
            if (data.car.flag === 'YELLOW') { setStyle(flagBox, 'backgroundColor', 'var(--fia-yellow)'); setStyle(flagBox, 'color', '#000'); }
            else if (data.car.flag === 'RED') { setStyle(flagBox, 'backgroundColor', 'var(--f1-red)'); setStyle(flagBox, 'color', '#FFF'); }
            else if (data.car.flag === 'BLUE') { setStyle(flagBox, 'backgroundColor', 'var(--fia-blue)'); setStyle(flagBox, 'color', '#000'); }
            ...
            if (data.penalties) {
                ...
                document.getElementById('lap-invalid-row').style.display = data.penalties.invalidLap === 1 ? 'flex' : 'none';
            }
            ...
            setText('record-driver', data.session.allTimeFastestDriver !== 'Unknown' ? `(${data.session.allTimeFastestDriver})` : '');
            ...
            setText('tyre-age-display', `${data.car.tyreAge} Laps`);

            setText('ers-mode', data.ers.mode);
            setStyleById('ers-battery', 'width', `${clampPercent(data.ers.battery)}%`);
            setText('ers-val', `${Math.round(data.ers.battery)}%`);
            setText('fuel', `${data.setup.fuel.toFixed(2)} kg`);
            setText('pit-limit', data.session.pitLimit);

            setText('wing-f', data.setup.wingF);
            setText('wing-r', data.setup.wingR);
            setText('diff-on', `${data.setup.diffOn}%`);
            setText('diff-off', `${data.setup.diffOff}%`);
            setText('camber-f', data.setup.camberF.toFixed(2));
            setText('camber-r', data.setup.camberR.toFixed(2));
            setText('toe-f', data.setup.toeF.toFixed(2));
            setText('toe-r', data.setup.toeR.toFixed(2));
            setText('bbias', `${data.setup.bBias}%`);

            if (data.leaderboard && data.leaderboard.length > 0) {
                const sessionType = data.session.type || '';
                const isTimeBased = sessionType.includes('Practice') || sessionType.includes('Q') || sessionType.includes('Time Trial');
                const isRace = sessionType.includes('Race') || sessionType.includes('Sprint');

                let headersHtml = `<span class="col-pos">POS</span><span class="col-driver">DRIVER</span>`;

                if (isTimeBased) {
                    headersHtml += `<span class="col-best">BEST</span>
                                    <span class="col-gap">GAP</span>
                                    <span class="col-laps">LAPS</span>
                                    <span class="col-stat">STAT</span>`;
                } else if (isRace) {
                    headersHtml += `<span class="col-tyre">TYRE</span>
                                    <span class="col-int">INT</span>
                                    <span class="col-laps">LAPS</span>`;
                } else {
                    headersHtml += `<span class="col-laps">LAPS</span>`;
                }

                setHtml('leaderboard-headers', headersHtml);

                let lbHtml = '';
                const statusMap = { 0: 'GARAGE', 1: 'FLYING', 2: 'IN LAP', 3: 'OUT LAP', 4: 'TRACK' };
                let leaderboardRows = Array.isArray(data.leaderboard) ? [...data.leaderboard] : [];

                if (isTimeBased) {
                    leaderboardRows.sort((a, b) => {
                        if (a.bestLapMs > 0 && b.bestLapMs > 0) return a.bestLapMs - b.bestLapMs;
                        if (a.bestLapMs > 0) return -1;
                        if (b.bestLapMs > 0) return 1;
                        return (a.carIndex || 0) - (b.carIndex || 0);
                    });

                    const leaderLapMs = leaderboardRows[0]?.bestLapMs || 0;
                    leaderboardRows = leaderboardRows.map((driver, idx) => {
                        return {
                            ...driver,
                            calcPos: idx + 1,
                            calcGapLeaderMs: leaderLapMs > 0 && driver.bestLapMs > 0 ? (driver.bestLapMs - leaderLapMs) : 0
                        };
                    });
                }

                leaderboardRows.forEach((driver, index) => {
                    const isFastest = driver.carIndex === data.session.fastestLapCarIndex && driver.bestLapMs > 0;
                    const isPlayer = driver.carIndex === data.playerIndex;
                    const nameColor = isPlayer ? '#fff' : (isFastest ? 'var(--fia-purple)' : 'var(--text-main)');
                    const bgCol = isPlayer ? 'rgba(255, 255, 255, 0.1)' : 'transparent';
                    const displayPos = isTimeBased ? (driver.calcPos || driver.pos || '--') : (driver.pos || '--');
                    const teamColor = getTeamColorCss(driver, data);

                    let driverName = (data.participants && data.participants[driver.carIndex]) ? data.participants[driver.carIndex].toUpperCase() : `CAR ${driver.carIndex}`;
                    const flBadge = isFastest ? '<span class="fl-icon">⏱</span>' : '';

                    let rowHtml = `<div class="lb-row" data-car-index="${driver.carIndex}" style="background: ${bgCol};">
                        <span class="val col-pos">${displayPos}</span>
                        <span class="val col-driver" style="color: ${nameColor};">
                            <span class="team-stripe" style="background: ${teamColor}; color: ${teamColor};"></span>
                            <span class="driver-name-text">${driverName}${flBadge}</span>
                        </span>`;

                    if (isTimeBased) {
                        let bestStr = driver.bestLapMs > 0 ? formatMs(driver.bestLapMs) : '--:--.---';
                        let gapStr = driver.calcPos === 1 ? 'LEAD' : (driver.calcGapLeaderMs > 0 ? `+${(driver.calcGapLeaderMs / 1000).toFixed(3)}` : '--');
                        let lapStr = driver.lapNum;

                        let statStr = driver.pitStatus === 1 || driver.pitStatus === 2 ? 'PIT' : (statusMap[driver.driverStatus] || 'TRACK');
                        let statCol = statStr === 'FLYING' ? 'var(--fia-purple)' : (statStr === 'PIT' ? 'var(--fia-yellow)' : 'var(--text-muted)');

                        rowHtml += `<span class="val col-best" style="color: ${isFastest ? 'var(--fia-purple)' : '#fff'};">${bestStr}</span>
                                    <span class="val col-gap">${gapStr}</span>
                                    <span class="val col-laps">${lapStr}</span>
                                    <span class="val col-stat" style="color: ${statCol};">${statStr}</span>`;
                    } else if (isRace) {
                        let compound = (driver.tyre || '-').split(' ')[0];
                        let intStr = index === 0 ? 'LEAD' : (driver.gapText || '--');
                        let lapStr = `${driver.lapNum}/${data.session.lapsTotal}`;

                        rowHtml += `<span class="val col-tyre" style="color: ${driver.tyreClass || '#FFF'};">${compound}</span>
                                    <span class="val col-int">${intStr}</span>
                                    <span class="val col-laps">${lapStr}</span>`;
                    } else {
                        rowHtml += `<span class="val col-laps">${driver.lapNum}</span>`;
                    }

                    rowHtml += `</div>`;
                    lbHtml += rowHtml;
                });
                animateClassificationSwap(
                    document.getElementById('leaderboard-container'),
                    lbHtml,
                    leaderboardRows.map(driver => String(driver.carIndex))
                );

                const playerRow = leaderboardRows.find(d => d.carIndex === data.playerIndex);
                setText('pos', isTimeBased ? (playerRow?.calcPos || '--') : (playerRow?.pos || '--'));

                const hHead = document.getElementById('history-head');
                const hBody = document.getElementById('history-body');
                let maxLaps = 0;
                leaderboardRows.forEach(d => { if (d.lapHistory && d.lapHistory.length > maxLaps) maxLaps = d.lapHistory.length; });

                if (maxLaps > 0) {
                    let headHtml = `<tr><th class="sticky-pos">POS</th><th class="sticky-name">DRIVER</th>`;
                    for (let i = 1; i <= maxLaps; i++) headHtml += `<th>LAP ${i}</th>`;
                    setElementHtml(hHead, headHtml + `</tr>`);

                    let bodyHtml = '';
                    leaderboardRows.forEach(driver => {
                        let driverName = (data.participants && data.participants[driver.carIndex]) ? data.participants[driver.carIndex].toUpperCase() : `CAR ${driver.carIndex}`;
                        let displayPos = isTimeBased ? (driver.calcPos || driver.pos || '--') : (driver.pos || '--');
                        const teamColor = getTeamColorCss(driver, data);

                        let tyreLetter = 'U';
                        if (driver.tyre) {
                            const uTyre = driver.tyre.toUpperCase();
                            if (uTyre.includes('SOFT')) tyreLetter = 'S'; else if (uTyre.includes('MEDIUM')) tyreLetter = 'M';
                            else if (uTyre.includes('HARD')) tyreLetter = 'H'; else if (uTyre.includes('INTER')) tyreLetter = 'I';
                        ...
                        for (let i = 0; i < maxLaps; i++) {
                            const lapDataObj = driver.lapHistory[i] || null;
                            const lapTimeMs = (lapDataObj && typeof lapDataObj === 'object') ? lapDataObj.lapTime : (lapDataObj || 0);
                            
                            let styleClass = "";
                            let onClickEvent = "";
                            let displayTime = formatMs(lapTimeMs);

                            if (lapTimeMs > 0) {
                                if (lapTimeMs === data.session.sessionFastestLapMs) styleClass = "time-sb";
                                else if (lapTimeMs === driver.bestLapMs) styleClass = "time-pb";
                                
                                if (lapDataObj && typeof lapDataObj === 'object') {
                                    if (lapDataObj.invalid) {
                                        styleClass += " invalid-lap";
                                        displayTime = `<span style="text-decoration: line-through; color: var(--f1-red);">${displayTime}</span>`;
                                    }
                                    onClickEvent = `onclick="showLapDetails('${driverName}', ${i+1}, ${lapDataObj.s1}, ${lapDataObj.s2}, ${lapDataObj.s3}, ${lapTimeMs}, ${lapDataObj.invalid})" style="cursor:pointer;" title="Click for sector breakdown"`;
                                }
                            }
                            rHtml += `<td class="${styleClass}" ${onClickEvent}>${displayTime}</td>`;
                        }
                        bodyHtml += rHtml + `</tr>`;
                    });
                ...
                setText('pos', data.lap.pos > 0 ? data.lap.pos : '--');
            }
        }

        function showLapDetails(driverName, lapNum, s1, s2, s3, total, invalid) {
            document.getElementById('ld-driver').textContent = driverName;
            document.getElementById('ld-lap').textContent = 'Lap ' + lapNum;
            document.getElementById('ld-s1').textContent = s1 > 0 ? (s1 / 1000).toFixed(3) : '--.---';
            document.getElementById('ld-s2').textContent = s2 > 0 ? (s2 / 1000).toFixed(3) : '--.---';
            document.getElementById('ld-s3').textContent = s3 > 0 ? (s3 / 1000).toFixed(3) : '--.---';
            document.getElementById('ld-total').textContent = formatMs(total);
            document.getElementById('ld-invalid').style.display = invalid ? 'block' : 'none';
            document.getElementById('lap-details-modal').style.display = 'flex';
        }
